...
class betatree(object):
    ...
    def estimate_MRCA_time(self, with_alpha=None):
        """
        For inner usage. Estimates current waiting time until the MRCA.
        :return: Waiting time
        """
        if with_alpha is None:
            ...
            alpha = with_alpha
        if alpha == 2:
            return 2 * (1 - 1/len(self.blocks))
        elif alpha == 1:
            return np.log(np.log((len(self.blocks))))
        else:
            print("Cannot estimate time!")
    ...
    def coalesce(self):
        ...
        whole_time = self.estimate_MRCA_time()
        while len(self.blocks) != 1:
            # print(self.alpha, len(self.blocks), round(self.blocks[0].branch_length, 2), self.estimate_MRCA_time(with_alpha=1), self.estimate_MRCA_time())
            if self.length_of_selection is not None:
                if self.alpha == 2:
                    # time_left_selection = self.estimate_MRCA_time(with_alpha=1)
                    # if time_left_selection <= self.length_of_selection * self.blocks[0].branch_length:
                    if self.n * self.length_of_selection >= len(self.blocks):
                        self.alpha = 1
                        self.inv_k = 1.0 / np.arange(1, self.n + 2)
                        self.inv_kkp1 = self.inv_k / np.arange(2, self.n + 3)
                        self.cum_sum_inv_kkp1 = np.array([0] + np.cumsum(self.inv_kkp1).tolist())
